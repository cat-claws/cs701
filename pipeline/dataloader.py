...
cls_labels_dict = {}
# {image_id: [one-hot labels]}
json_data = json.load(open("data/public/train.json", "r"))
for image_data in json_data:
    image_id = image_data['img_path'].split('/')[-1].split('.')[0]
    one_hot_labels = image_data['target']
    one_hot_labels = [_label * 1.0 for _label in one_hot_labels]
    cls_labels_dict[image_id] = np.array(one_hot_labels)

# still need (predicted) labels for the validation/test set.
json_data = json.load(open("data/public/val.json", "r"))
for image_data in json_data:
    image_id = image_data['img_path'].split('/')[-1].split('.')[0]
    one_hot_labels = image_data['target']
    one_hot_labels = [_label * 1.0 for _label in one_hot_labels]
...
def load_img_name_list(json_file_path):
    json_data = json.load(open(json_file_path, "r"))
    img_name_list = []
    for image_data in json_data:
        image_name = image_data['img_path'].split('/')[-1].split('.')[0]
...
class VOC12ImageDataset(Dataset):
    ...
    def __init__(self, json_file_path, voc12_root,
                 resize_long=None, rescale=None, img_normal=TorchvisionNormalize(), hor_flip=False,
                 crop_size=None, crop_method=None, to_torch=True):

        self.img_name_list = load_img_name_list(json_file_path)
...
class VOC12ClassificationDataset(VOC12ImageDataset):
    ...
    def __init__(self, json_file_path, voc12_root,
        ...
        super().__init__(json_file_path, voc12_root,
...
class VOC12ClassificationDatasetMSF(VOC12ClassificationDataset):
    ...
    def __init__(self, json_file_path, voc12_root,
        ...
        super().__init__(json_file_path, voc12_root, img_normal=img_normal)
        self.scales = scales
    ...
    def __getitem__(self, idx):
        ...
        out = {"name": name_str, "img": ms_img_list, "size": (img.shape[0], img.shape[1]),
               "label": torch.from_numpy(self.label_list[idx]), "img_path":get_img_path(name_str, self.voc12_root)}
        return out

class VOC12SegmentationDataset(Dataset):

    def __init__(self, json_file_path, label_dir, crop_size, voc12_root,
...
class VOC12AffinityDataset(VOC12SegmentationDataset):
    def __init__(self, json_file_path, label_dir, crop_size, voc12_root,
        ...
        super().__init__(json_file_path, label_dir, crop_size, voc12_root, rescale, img_normal, hor_flip, crop_method=crop_method)
